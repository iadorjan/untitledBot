...
function getRootstockBlockNumber(): Promise<string> {
...
function getRootstockBalance(address: string): Promise<{ wei: string; ether: string }> {
  const postData = JSON.stringify({
    jsonrpc: "2.0",
...
function getHederaBalance(accountId: string): Promise<{ tinybars: string; hbar: string }> {
      ...
      .get(url, (res) => {
        ...
        res.on("end", () => {
          try {
            ...
            let tinybars: bigint | null = null;
            if (balObj && typeof balObj === "object") {
              if (typeof balObj.balance !== "undefined") tinybars = BigInt(balObj.balance);
            }
            if (tinybars === null && parsed && typeof parsed.balance === "number") tinybars = BigInt(parsed.balance);
            if (tinybars === null) {
              if (parsed && typeof parsed.balance === "string") tinybars = BigInt(parsed.balance);
            }

            if (tinybars === null) return reject(new Error("Unable to parse Hedera balance from response: " + raw));
...
async function getWorldBalanceRPC(address: string): Promise<{ wei: string; ether: string }> {
  const rpcWs = process.env.RPC_URL_WORLD_WS || process.env.RPC_URL_WORLD;
  if (!rpcWs) throw new Error("No World RPC URL configured (RPC_URL_WORLD_WS or RPC_URL_WORLD)");

  let providerWeb3: Web3;
  try {
    if (rpcWs.startsWith("ws://") || rpcWs.startsWith("wss://")) {
      providerWeb3 = new Web3(new Web3.providers.WebsocketProvider(rpcWs));
    } else {
      providerWeb3 = new Web3(rpcWs);
    }
    ...
    try {
      const prov: any = providerWeb3.currentProvider;
      if (prov && typeof prov.disconnect === "function") prov.disconnect();
